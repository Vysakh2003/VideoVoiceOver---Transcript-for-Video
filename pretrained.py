...
def generate_caption(total_frames):
  ...
  while curr_frame < total_frames:
    ...
    produced_caption = get_caption(model, image_processor, tokenizer, url)
    if produced_caption not in caption_list:
        caption_list.append(produced_caption)
    print(caption_list)
    curr_frame += 30
  return caption_list
...
def summarize_text(text):
    return summarize(text,ratio=0.4).replace("\n","")
